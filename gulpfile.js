...
// 拷贝图片至dist
gulp.task('copyImg', () => {
    return gulp.src(['src/images/*', 'src/images/*/*.*', 'src/img-not-hash/*', 'src/img-not-hash/*/*.*'])
...
gulp.task('revHtmlImg', () => { // 替换html中的img为hash img
    return gulp.src(['rev/images/*.json', 'build/html/*.html'])
        .pipe(revCollector({
            replaceReved: true,
            dirReplacements: {
...
gulp.task('watch', () => {
    ...
    gulp.watch(['src/images/*.*', 'src/images/*/*.*'], ['copyImg'])
})
