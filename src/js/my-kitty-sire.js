...
$(function () {
    ...
    $('.myOwnDogSire strong').text(dogInfo.nickname)
    ...
    $('body').on('click', '.payModal .Section .finalPay', function () {
        ...
        if ($password.val().trim() === '') {
            ...
            if (bytes && bytes.trim() !== '') {
                // 獲取掛狗交易單簽名
                // sellSign = JSON.stringify(ACT.callContract(bytes, 'addMatingTransaction', `${id}|${$startprince}|${$endprince}|${$duration}`, transParams))
    ...
    $('.Tabs--center ul li').on('click', function (e) {
        $(this).addClass('is-active').siblings().removeClass('is-active')
        $(this).parent('ul').next('.Tabs-panes').find('.Tabs-pane').eq($(this).index()).css('display', 'block').siblings().css('display', 'none')
    })
})
